...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Sora:wght@400;600;700;800&family=Geist:wght@400;500;600&display=swap" rel="stylesheet"/>
...
<body class="bg-background text-on-surface font-body-md overflow-hidden">
    ...
    <main class="relative min-h-screen flex items-center justify-center lg:justify-end lg:pr-margin-desktop overflow-hidden">
        ...
        <div class="hidden lg:flex flex-col absolute left-margin-desktop top-1/2 -translate-y-1/2 z-10 max-w-2xl pointer-events-none">
            <div class="mb-unit">
                <span class="bg-primary-container text-white px-3 py-1 font-label-md text-label-md tracking-widest uppercase font-bold">Join the Scuderia</span>
            </div>
        ...
        <section class="relative z-20 w-full max-w-[460px] px-margin-mobile lg:px-0 animate-fade-in">
            <div class="glass-panel p-8 md:p-12 shadow-2xl relative overflow-hidden">
                ...
                <div class="flex gap-8 mb-8 border-b border-surface-variant">
                    <a class="pb-4 font-label-md text-label-md text-on-surface-variant hover:text-on-surface uppercase tracking-wider" href="${pageContext.request.contextPath}/login.jsp">Đăng nhập</a>
                    <a class="pb-4 font-label-md text-label-md uppercase tracking-wider text-primary-container border-b-2 border-primary-container font-bold" href="${pageContext.request.contextPath}/register.jsp">Đăng ký</a>
                </div>
                ...
                <form action="${pageContext.request.contextPath}/register" method="post" class="space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="font-label-md text-label-md text-on-surface-variant uppercase tracking-widest block">Tên họ</label>
                            <input name="txtLastName" class="w-full bg-surface-container-lowest border border-surface-variant px-4 py-3 text-on-surface font-mono-data focus:outline-none transition-all placeholder:text-surface-variant" placeholder="FERRARI" required type="text"/>
                        </div>
                        <div class="space-y-2">
                            <label class="font-label-md text-label-md text-on-surface-variant uppercase tracking-widest block">Tên gọi</label>
                            <input name="txtFirstName" class="w-full bg-surface-container-lowest border border-surface-variant px-4 py-3 text-on-surface font-mono-data focus:outline-none transition-all placeholder:text-surface-variant" placeholder="ENZO" required type="text"/>
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="font-label-md text-label-md text-on-surface-variant uppercase tracking-widest block">Số điện thoại liên lạc</label>
                        <input name="txtPhone" class="w-full bg-surface-container-lowest border border-surface-variant px-4 py-3 text-on-surface font-mono-data focus:outline-none transition-all placeholder:text-surface-variant" placeholder="0912345678" required type="tel"/>
                    </div>
                    <div class="space-y-2">
                        <label class="font-label-md text-label-md text-on-surface-variant uppercase tracking-widest block">Địa chỉ Email</label>
                        <input name="txtEmail" class="w-full bg-surface-container-lowest border border-surface-variant px-4 py-3 text-on-surface font-mono-data focus:outline-none transition-all placeholder:text-surface-variant" placeholder="enzo@ferrari.it" required type="email"/>
                    </div>
                    <div class="space-y-2">
