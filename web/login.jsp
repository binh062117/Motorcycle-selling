...
<fmt:message key="login.pageTitle" var="loginPageTitle" />
<!DOCTYPE html>
...
<body class="bg-dark text-white overflow-hidden">
    <main class="position-relative min-vh-screen d-flex align-items-center justify-content-center justify-content-lg-end pe-lg-5 overflow-hidden" style="min-height: 100vh;">
        ...
        <div class="position-absolute start-0 top-50 translate-middle-y z-1 d-none d-lg-flex flex-column max-w-2xl px-5 pointer-events-none">
            <div class="mb-2">
                <span class="bg-danger text-white px-3 py-1 font-heading text-uppercase small fw-bold"><fmt:message key="login.heroEyebrow" /></span>
            </div>
            <h1 class="font-heading display-3 text-white mb-4 text-uppercase fw-black" style="font-weight: 900;"><fmt:message key="login.heroTitle" /></h1>
            <p class="text-secondary fs-5" style="max-width: 500px;"><fmt:message key="login.heroBody" /></p>
        </div>
        ...
        <section class="position-relative z-2 w-100 px-3" style="max-width: 460px;">
            <div class="glass-panel p-4 p-md-5 shadow-lg position-relative overflow-hidden racing-stripe">
                ...
                <div class="d-flex gap-4 mb-4 border-bottom border-secondary">
                    <a class="pb-3 font-heading text-uppercase tracking-wider small text-danger border-bottom border-3 border-danger fw-bold text-decoration-none" href="${pageContext.request.contextPath}/login.jsp"><fmt:message key="login.tab" /></a>
                    <a class="pb-3 font-heading text-uppercase tracking-wider small text-secondary hover-text-white text-decoration-none" href="${pageContext.request.contextPath}/register.jsp"><fmt:message key="register.tab" /></a>
                </div>
                ...
                <c:if test="${not empty successMessage}">
                     <div class="alert alert-success rounded-0 font-mono-data text-xs text-uppercase py-2" role="alert">
                         ${successMessage}
                     </div>
                </c:if>
                ...
                <form action="${pageContext.request.contextPath}/login" method="post" class="needs-validation-custom d-flex flex-column gap-3" novalidate>
                    <div class="form-group">
                        <label class="font-heading text-muted text-uppercase tracking-wider small d-block mb-2"><fmt:message key="login.email" /></label>
                        <input name="txtEmail" class="form-control bg-black border-secondary text-white font-mono-data rounded-0" placeholder="name@ducatishowroom.com" required type="email"/>
                    </div>
                    <div class="form-group">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <label class="font-heading text-muted text-uppercase tracking-wider small d-block m-0"><fmt:message key="login.password" /></label>
                            <a class="font-heading text-danger small text-decoration-none" href="${pageContext.request.contextPath}/forgot-password"><fmt:message key="login.forgotPassword" /></a>
                        </div>
                    ...
                    <div class="form-check d-flex align-items-center gap-2">
                        ...
                        <label class="form-check-label text-muted small" for="remember"><fmt:message key="login.remember" /></label>
                    </div>

                    <button class="btn btn-ducati w-100 py-3 mt-2 d-flex align-items-center justify-content-center gap-2" type="submit">
                        <span><fmt:message key="login.submit" /></span>
                    ...
                    <a href="${pageContext.request.contextPath}/google-login" class="btn btn-outline-ducati w-100 py-3 d-flex align-items-center justify-content-center gap-2" style="font-size:12px;">
                        ...
                        <span><fmt:message key="login.google" /></span>
                    </a>
                </form>

                <div class="mt-4 text-center">
                    <p class="text-secondary small m-0">
                        <fmt:message key="login.backPrefix" /> <a class="text-white underline fw-bold text-decoration-none border-bottom border-white pb-1" href="${pageContext.request.contextPath}/index.jsp"><fmt:message key="login.backHome" /></a>.
                    </p>
                </div>
            </div>
        </section>
