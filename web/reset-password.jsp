...
<c:set var="activeResetToken" value="${not empty resetToken ? resetToken : param.token}" />
<!DOCTYPE html>
<html lang="${empty sessionScope.appLang ? 'vi' : sessionScope.appLang}">
<head>
    <meta charset="utf-8"/>
    <meta content="width=device-width, initial-scale=1.0" name="viewport"/>
    <title>DUCATI SHOWROOM | Đặt Lại Mật Khẩu</title>
    ...
    <main class="position-relative min-vh-screen d-flex align-items-center justify-content-center justify-content-lg-end pe-lg-5 overflow-hidden" style="min-height: 100vh;">
        ...
        <section class="position-relative z-2 w-100 px-3" style="max-width: 460px;">
            <div class="glass-panel p-4 p-md-5 shadow-lg position-relative overflow-hidden racing-stripe">
                ...
                <c:choose>
                    <c:when test="${empty activeResetToken}">
                        <p class="text-secondary small mb-4">Vui lòng xác minh email và số điện thoại trước khi đổi mật khẩu.</p>
                        <a class="btn btn-ducati w-100 py-3 text-uppercase" href="${pageContext.request.contextPath}/forgot-password">Xác minh tài khoản</a>
                    </c:when>
                    <c:otherwise>
                        <p class="text-secondary small mb-4">Tài khoản đã được xác minh. Nhập mật khẩu mới và xác nhận lại để hoàn tất.</p>
                        ...
                        <form action="${pageContext.request.contextPath}/reset-password" method="post" class="needs-validation-custom d-flex flex-column gap-3" novalidate>
                            <input type="hidden" name="token" value="${activeResetToken}"/>
                            ...
                            <div class="form-group">
                                ...
                                <input name="txtConfirmPassword" id="confirmPassword" class="form-control bg-black border-secondary text-white font-mono-data rounded-0" placeholder="********" required type="password" minlength="6"/>
                            </div>
