...
<!DOCTYPE html>
<html lang="${empty sessionScope.appLang ? 'vi' : sessionScope.appLang}">
<head>
    <meta charset="utf-8"/>
    <meta content="width=device-width, initial-scale=1.0" name="viewport"/>
    <title>DUCATI SHOWROOM | Quên Mật Khẩu</title>
        ...
        <section class="position-relative z-2 w-100 px-3" style="max-width: 460px;">
            <div class="glass-panel p-4 p-md-5 shadow-lg position-relative overflow-hidden racing-stripe">
                <div class="d-flex flex-column align-items-center align-items-lg-start mb-4 select-none">
                    <span class="font-heading fs-4 fw-black text-danger tracking-tighter uppercase leading-none m-0" style="font-weight: 900;">QUÊN MẬT KHẨU</span>
                    <div class="bg-danger mt-2" style="width: 48px; height: 3px;"></div>
                </div>

                <p class="text-secondary small mb-4">Nhập đúng email và số điện thoại đã đăng ký để mở form đổi mật khẩu.</p>

                <c:if test="${not empty errorMessage}">
                    <div class="alert alert-danger rounded-0 font-mono-data text-xs text-uppercase py-2" role="alert">${errorMessage}</div>
                </c:if>

                <form action="${pageContext.request.contextPath}/forgot-password" method="post" class="needs-validation-custom d-flex flex-column gap-3" novalidate>
                    <div class="form-group">
                        <label class="font-heading text-muted text-uppercase tracking-wider small d-block mb-2">Địa chỉ Email</label>
                        <input name="txtEmail" class="form-control bg-black border-secondary text-white font-mono-data rounded-0" placeholder="name@ducatishowroom.com" required type="email" value="${param.txtEmail}"/>
                    </div>
                    <div class="form-group">
                        <label class="font-heading text-muted text-uppercase tracking-wider small d-block mb-2">Số điện thoại đăng ký</label>
                        <input name="txtPhone" class="form-control bg-black border-secondary text-white font-mono-data rounded-0 validate-phone" placeholder="0912345678" required type="tel" value="${param.txtPhone}"/>
                    </div>
                ...
                <div class="mt-4 text-center">
                    <p class="text-secondary small m-0">
                        Quay lại <a class="text-white underline fw-bold text-decoration-none" href="${pageContext.request.contextPath}/login">Đăng nhập</a>.
                    </p>
                </div>
            </div>
        </section>
